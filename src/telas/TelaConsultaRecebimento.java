...
public class TelaConsultaRecebimento extends JInternalFrame implements MouseListener {
    ...
    public JLabel labelValorContaReceber = new JLabel("");
    ...
    public TelaConsultaRecebimento(ParametrosConsulta parametrosConsulta) {
        ...
        tabela = new JTable(dtm) {
            ...
            public Component prepareRenderer(TableCellRenderer renderer, int rowIndex, int vColIndex) {
                ...
                try {
                    ...
                    Logger.getLogger(TelaConsultaRecebimento.class.getName()).log(Level.SEVERE, null, ex);
                }
		Date dataAtual = null;
		...
		String aux1 = formato.format(dataHoje);
                ...
                if (dataVencimento.before(dataAtual)) { //verfifica se a data é anterior a data atual, e então pinta as letras de vermelho indicando que está vencida
                    ...
                    c.setForeground(Color.decode("#fce641"));
        ...
        painelBaixo.add(labelValorContaReceber);    
    ...
    private void aplicaRender() {
        ...
        tableModel.getColumn(1).setMinWidth(350); //tamanho da coluna 1 (geralmente onde está os maiores campos) em todas as telas de consulta.
        tableModel.getColumn(1).setMaxWidth(350); //tamanho da coluna 1 (geralmente onde está os maiores campos) em todas as telas de consulta.
                    ...
                    public void run() {
                        if (textoPesquisar.getText().isEmpty()) {
                            while (dtm.getRowCount() > 0) {
                                ...
                                labelValorContaReceber.setText("");
                            }
                            jcbOrdenar.setEnabled(false);
                            return;
                        }
    ...
    private void preencherDadosPesquisar() {
        ...
        if (jcbPesquisar.getSelectedIndex() > 0) {
            ...
            if (tipo == Date.class && jcbPesquisar.getSelectedIndex() == 5) {
                ...
                tempSql = tempSql +
                        "CAST(" +
                        "LPAD(EXTRACT(DAY FROM CONTARECEBER_DATA), 2, '0') || '/' || " +
                        "LPAD(EXTRACT(MONTH FROM CONTARECEBER_DATA), 2, '0') || '/' || " +
                        "LPAD(EXTRACT(YEAR FROM CONTARECEBER_DATA), 4, '0') " +
                        "AS VARCHAR(10)) " +
                        "LIKE '%";
                tempSql = tempSql + textoPesquisar.getText().toUpperCase()
                        + "%'";
    ...
    private void preencherDadosListar() {
        String tempSql = parametrosConsulta.getSql();
        guardaSql = tempSql;
        tempSql = parametrosConsulta.getSql() + " ORDER BY PARCELACONTARECEBER.VENCIMENTO";
    ...
    private void somaValorReceber() {
        ...
        labelValorContaReceber.setText("Valor total a receber: " + NumberFormat.getCurrencyInstance().format(soma));
    }
